...
```text
bookloop/auth.py, api.py, admin/  → 인증·JSON·관리자 제품 adapter
bookloop/clients/, services/      → 제품 화면 adapter와 공통 업무 규칙
bookloop/db/                      → 제품과 개발 도구가 공유하는 persistence 경계
...
| 폴더 | 역할 | 상태 |
| --- | --- | --- |
| [`db_inspector/`](db_inspector/README.md) | User·BookListing·BorrowRequest를 읽기 전용으로 확인 | v0.1 |
...
필요해져도 `db_inspector`에 섞지 않고 `bl_cli/seed/`의 별도 책임으로 유지한다.
수정·삭제 기능은 개발 편의보다 데이터 안전성과 현재 D3 제품·증거 경계를 먼저
검토한 뒤 결정한다.
