...
브라우저 Reset 버튼은 실수로 demo evidence를 삭제할 수 있고, read-only 계약을 깨며,
CSRF 보호·POST·확인 절차와 감사 기록이 필요하다. 따라서 현재는 Inspector에 reset
명령을 안내만 하고 실제 변경은 터미널 BL-CLI에서 수행한다.
...
- [ ] 데이터 변경은 명시적인 BL-CLI 명령으로만 실행했다.
