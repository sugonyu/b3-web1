...
| 문서 | 역할 |
| --- | --- |
| [`README.html`](README.html) | 최신 `BookLoop D3 · Code Map v1.8` |
| [`../docs/archive/README-v1.6-d3.html`](../docs/archive/README-v1.6-d3.html) | D3 v1.6 frozen archive |
| [`../docs/archive/README-v1.5-d3.html`](../docs/archive/README-v1.5-d3.html) | D3 v1.5 frozen archive |
| [`../docs/archive/README-v1.4-d3.html`](../docs/archive/README-v1.4-d3.html) | D3 v1.4 frozen archive |
| [`../docs/archive/README-v1.3-d3.html`](../docs/archive/README-v1.3-d3.html) | D3 v1.3 frozen archive |
| [`../docs/archive/README-v1.2-d2.html`](../docs/archive/README-v1.2-d2.html) | D2 v1.2 frozen archive |
| [`../docs/archive/README-v1.1-d2.html`](../docs/archive/README-v1.1-d2.html) | D2 v1.1 frozen archive |
| [`../docs/archive/README-v1.0-d2.html`](../docs/archive/README-v1.0-d2.html) | D2 v1.0 frozen archive |
| [`../docs/development/CODE_OUTLINE_MANUAL.md`](../docs/development/CODE_OUTLINE_MANUAL.md) | PyCharm 학습용 파일 연결·함수 아웃라인 매뉴얼 |
| [`../BOOKLOOP_DOCUMENTATION_INDEX.md`](../BOOKLOOP_DOCUMENTATION_INDEX.md) | 사용자·운영자·개발자 문서 인덱스 |
| [`../docs/development/API_CONTRACT.md`](../docs/development/API_CONTRACT.md) | JSON API와 privacy boundary 계약 |

`README.html`은 계속 갱신하는 canonical latest Code Map이고, version suffix가 있는
HTML은 해당 시점의 구조를 보존하는 archive다. archive는 최신 코드에 맞춰 다시
쓰지 않는다.

현재 자동화 검증 기준은 **최근 전체 검증 159 backend tests passed**다. 문서 변경
후에는 관련 focused test와 전체 suite를 다시 실행해 이 수치를 확인한다.
...
| --- | --- | --- |
| `jinja_client` | `/`, `/books/*`, `/requests/*`, `/users/*`, `/jinja/` | 제품 UI와 Jinja reference |
...
Admin UI는 system overview, book-sharing state와 Report review queue를 보여준다.
...
Books table summary
→ /books/<listing_id>
→ public BookListing details
→ login / existing request / optional message request CTA
```

`book_detail.html`은 기존 `BookListing` 데이터만 사용해 제목·저자·소유자·공개 지역·상태·
등록일·Book ID를 보여준다. 이메일이나 승인 전 연락처 같은 private data는 포함하지 않는다.

## 7. Intentional boundaries and known gaps

- `devtools/`는 제품 업무 규칙을 우회하지 않는 local-only 도구다.
- DB Inspector의 표와 Report 데이터는 read-only이며, 로컬 Reset 버튼은 확인 후 demo
  BorrowRequest와 연결되거나 남겨진 orphan Report를 삭제하고 demo listing availability를
  복원한다. 전체 데이터
  초기화는 제공하지 않으며 BL-CLI 명령도 유지한다.
...
```text
Public Book details
→ public BookListing read service
→ privacy-safe detail template
→ request CTA or login handoff
