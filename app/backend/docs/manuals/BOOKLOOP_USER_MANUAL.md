...
화면 오른쪽 위의 `🌙 Dark mode` 버튼으로 어두운 색상 테마를 켤 수 있다.
선택은 browser의 `localStorage`에 저장되므로 다음 방문에도 유지된다.

```text
🌙 Dark mode → 어두운 테마로 전환
☀️ Light mode → 밝은 테마로 전환
```

Dark mode는 색상과 대비만 바꾸며, request·Report·Profile 데이터와 권한에는
영향을 주지 않는다. 버튼과 Status 텍스트를 함께 확인하고 색상만으로 상태를
판단하지 않는다.
