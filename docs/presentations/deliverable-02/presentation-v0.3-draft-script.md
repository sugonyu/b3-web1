...
> Based on: `presentation-v0.3-draft.html`
> Presenter: Tony Yu
...
Kamyar's main feedback was to prove privacy in a meaningful way and to consider
verification, reporting, and an Admin View. D2 begins that response with protected
request access. Report entry and the protected Admin boundary remain later roadmap
steps, not completed D2 features.
...
- Kamyar 피드백을 `D2 protected access → D3 report → D4 Admin`으로 짧게 연결한다.
...
This is my first practical response to Kamyar's privacy feedback. The server allows
only the borrower and the book owner to view the request. It blocks unrelated users
...
- 이 authorization boundary가 privacy 피드백에 대한 첫 기술적 응답임을 강조한다.
...
The future Report service can reuse authentication, validation, persistence, and
safe responses. The future Admin View adds a role-protected review boundary. It
does not mean unrestricted access to private data.
...
- Report와 Admin 카드는 future scope이며 D2 완료 기능이 아니라고 말한다.
...
The privacy roadmap continues in small steps: D2 builds the privacy foundation,
D3 connects React and defines the smallest report entry, D4 adds a protected Admin
View, and the final presentation demonstrates the verified privacy flow.
...
- `D2 → D3 → D4 → Final` privacy roadmap을 한 문장으로 연결한다.
...
### Why are the Report system and Admin View not implemented in D2?

> I first need a verified authentication and authorization foundation. Report and
> Admin features require the same protected service, role, validation, and database
> boundaries. I will add them in later milestones instead of presenting an
> incomplete privacy feature as finished.
...
- [ ] Keep Report and Admin View clearly labelled as future roadmap work.
