...
현재는 홈 네트워크 개발 단계라 Rose LAN 검수를 위해 `ENABLE_LAN_DEV_USER_SWITCHER`
opt-in을 사용할 수 있다. 이 설정은 운영 공개용이 아니며, 배포 전에 반드시 false로
돌린다.
...
# Rose LAN 개발 단계에서 FLASK_DEBUG=false로 실행할 때 추가
ENABLE_LAN_DEV_USER_SWITCHER=true
```

route는 기본적으로 `DEBUG=true`와 `ENABLE_DEV_USER_SWITCHER=true`가 충족될 때
동작한다. Rose 홈 네트워크 개발 단계에서는 `ENABLE_LAN_DEV_USER_SWITCHER=true`를
추가로 명시할 수 있다. 두 opt-in이 없으면 `404`를 반환하고 배너도 렌더링하지 않는다.
