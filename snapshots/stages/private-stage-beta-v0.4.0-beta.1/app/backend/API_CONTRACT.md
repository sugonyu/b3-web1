...
> D3 React handoff contract · portability boundary updated 2026-08-09
...
## Implementation portability boundary

D3에서는 Flask가 canonical backend다. 그러나 이 계약은 Flask decorator,
SQLAlchemy query 문법이나 `jsonify()` 사용법이 아니라 client가 관찰하는 URL,
method, JSON shape, status code, authorization과 privacy rule을 정의한다.

따라서 D3 이후 Express가 같은 계약을 재현하더라도 React component는 바꾸지 않고
API base URL만 전환할 수 있어야 한다.

```text
⚛️ React API client
├── VITE_API_BASE_URL=http://127.0.0.1:5000/api  → Flask
└── VITE_API_BASE_URL=http://127.0.0.1:3001/api  → Express
```

현재 Express sibling은 contract 학습용 `GET /api/health`만 구현한다. 제품 endpoint는
여전히 Flask가 canonical이며, Listing·BorrowRequest·Reporting API를 Express에
복제하지 않는다. Flask route를 thin HTTP adapter로, validation·authorization과
workflow를 service 경계로 유지하는 보완을 계속한다. 미래 Express sibling 위치와
단계별 중단선은
