...
- 사용자 `tony` — 일반 회원이자 Homer listing 두 권의 owner
- 사용자 `mina` — 일반 회원이자 Han Kang listing 두 권의 owner
- 사용자 `alex` — 책 목록이 없는 유일한 관리자이자 authorization boundary 확인용 계정
...
관리자 권한도 username 문자열로 판단하지 않는다. 시드는 Alex의 DB `is_admin`만
`true`, Tony와 Mina는 `false`로 동기화한다.
